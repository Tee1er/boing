...
let get_server = async function() {
    ...
    const release_metadata = await axios.default.get(data.SETTINGS.serverResource, { responseType: "json" })
        .then(res => res.data)
        .catch(err => console.error(err));

    if ((!fs.existsSync(SERVER_JAR)) || fs.readFileSync(SERVER_JAR).toString() == "" || data.SESSION_DATA.server_release_id != release_metadata.id) {
        ...
        await axios.default.get(server_release["browser_download_url"], { responseType: "arraybuffer" })
            .then(result => {
