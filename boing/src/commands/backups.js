...
let execute = async function (interaction) {
	...
	for (let i = 0; i < backups.length; i++) {
		element = backups[i];
		embed.addFields({
			name: `[${backups.length - i}] ${element.map} - Wave ${
				element.wave
			}`,
			value: `${new Date(element.time)
				.getHours()
				.toString()
				.padStart(2, "0")}:${new Date(element.time)
				.getMinutes()
				.toString()
				.padStart(2, "0")}, ${new Date(element.time).toDateString()}`,
		});
	}
