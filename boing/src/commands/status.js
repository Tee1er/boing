...
let execute = async function (interaction) {
	...
	let value = await mserver.write_poll(
		"status",
		(line) =>
			line.includes("server closed") ||
			line.includes("players connected.") ||
	...
	let percentTimeRunning =
		(data.SESSION_DATA.uptime.msRunning / msSinceFirstStart) * 100;
	...
	interaction.editReply(
		"```js\n" +
			...
			"``` \n **Uptime:**\n" +
			`> ${percentTimeRunning}. \n> Total runtime: ${(
				new Date(data.SESSION_DATA.uptime.msRunning) / 3600000
			).toFixed(2)} hrs.`
	);
};
