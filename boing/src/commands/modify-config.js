...
let execute = async function (interaction) {
    ...
    if (configOption) {
        if (!VALID_CONFIGS.includes(configOption)) {
            interaction.reply(`Option ${configOption} not found or disabled. Use the 'list-config' command to see all available options`)
        };

        if (configValue) {
            result = await mserver.write_recv(`config ${configOption} ${configValue}`).then(res => `Server option ${res.match(regexes.message)}`);
...
module.exports = {
    ...
    info: new SlashCommandBuilder()
        ...
        .setDescription("List all configuration options for the server and their values, or modify a specific option.")
